...
/// trailing edge. Tapping it expands the full pill out to its leading side; tapping it again
/// collapses back down. Picking a different tab from the expanded pill doesn't auto-collapse
/// — collapsing is a deliberate, separate action from navigating.
struct FloatingTabBar: View {
    ...
    private var toggleCircle: some View {
        Button {
            ...
            Image(systemName: isExpanded ? "xmark" : selection.icon)
                .font(.system(size: 19, weight: .bold))
                .foregroundColor(isExpanded ? .white.opacity(0.75) : selection.accent)
                .frame(width: circleDiameter, height: circleDiameter)
        }
        .buttonStyle(.plain)
        .glassCard(cornerRadius: circleDiameter / 2, tint: isExpanded ? .white : selection.accent)
